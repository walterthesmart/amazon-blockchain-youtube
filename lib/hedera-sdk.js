...
// Conditionally import Hedera SDK only in browser
let HederaSDK = {}
...
if (isBrowser) {
  try {
    ...
    console.warn('Hedera SDK not available in browser:', error)
  }
}

// Extract classes from SDK or provide fallbacks
const {
  Client = class MockClient {},
  AccountId = class MockAccountId { static fromString() { return {} } },
  PrivateKey = class MockPrivateKey { static fromString() { return {} } },
  ContractCallQuery = class MockContractCallQuery {},
  ContractExecuteTransaction = class MockContractExecuteTransaction {},
  ContractFunctionParameters = class MockContractFunctionParameters {},
  Hbar = class MockHbar { static fromTinybars() { return {} } },
  TransactionId = class MockTransactionId {},
  AccountBalanceQuery = class MockAccountBalanceQuery {},
  TokenCreateTransaction = class MockTokenCreateTransaction {},
  TokenType = { FungibleCommon: 'FUNGIBLE_COMMON' },
  TokenSupplyType = { Infinite: 'INFINITE' },
  TokenMintTransaction = class MockTokenMintTransaction {},
  TokenBurnTransaction = class MockTokenBurnTransaction {},
  TokenAssociateTransaction = class MockTokenAssociateTransaction {},
...
export const initializeHederaClient = (network = 'testnet', operatorId = null, operatorKey = null) => {
  if (!isBrowser) {
    console.warn('Hedera client can only be initialized in browser environment')
    return null
  }
...
export const getAccountBalance = async (accountId) => {
  if (!isBrowser) {
    console.warn('Account balance can only be queried in browser environment')
    return { hbar: '0', tokens: {} }
  }

  try {
    const client = getHederaClient()
    if (!client) return { hbar: '0', tokens: {} }
